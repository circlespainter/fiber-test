...
public class ObjectMailboxFiberWorker extends AbstractMailboxFiberRingWorker<Channel<Integer>> {
    public ObjectMailboxFiberWorker(final FiberScheduler scheduler, final int id, final String name, final int[] seqs, final CountDownLatch cdl, final int mboxSize, final Channels.OverflowPolicy mboxPolicy, final Blackhole bh) {
        super(scheduler, id, name, seqs, cdl, Channels.newChannel(mboxSize, mboxPolicy), bh);
    }
